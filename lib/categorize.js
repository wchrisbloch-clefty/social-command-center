...
/**
 * Under this much text there is not enough to make an episode-level claim.
 * ASSUMPTION: 200 characters is roughly one sentence of show notes. Shared with
 * the podcasts route, which uses the same number to decide whether a summary
 * can be written at all — the two questions have the same answer.
 */
export const THIN_NOTES_CHARS = 200;

/**
 * Which TOPIC category this episode belongs to.
 *
 * ── WHY AN EPISODE HAS ITS OWN CATEGORY ─────────────────────────────────────
 * A show has one category; its episodes do not. Acquired is a business show
 * and its episode on a chipmaker is also a tech story. Filing every episode
 * under its show's category would make the Tech tab silently wrong — the
 * episode is there in the Podcasts tab and absent from the feed where someone
 * looking for chip news would find it.
 *
 * So the episode is classified on the text the publisher actually gave us, with
 * the show's wired category as a tie-breaking hint (categorize() weights a hint
 * at +1.5, so a business show's ambiguous episode still leans business).
 *
 * ── WHAT HAPPENS WHEN THE TEXT WILL NOT SUPPORT A CALL ──────────────────────
 * Nothing is force-assigned. Three outcomes, in order:
 *
 *   confident (high/medium)  → the classified category
 *   thin notes, no call      → `general`, which is the everything page rather
 *                              than a topic tab. The episode stays visible in
 *                              the Podcasts tab and in All signal, and does not
 *                              appear under a subject nobody established.
 *   enough text, still no call → the SHOW's wired category
 *
 * ASSUMPTION: that last case falls back to the show rather than to `general`
 * because the show's category is a human decision already made, not a guess.
 * Discarding it would be throwing away better information than the classifier
 * had. Thin notes are different in kind: there is no text to read, so there is
 * no episode-level claim to make at all.
 *
 * `categorySource` records which of the three happened, so a surprising
 * placement is inspectable rather than mysterious.
 */
export function classifyEpisode(ep, showCategory, { thinNotesChars = THIN_NOTES_CHARS } = {}) {
  const notes = ep.notes || ep.desc || '';
  const text = `${ep.title || ''} ${notes}`;
  const thin = notes.length < thinNotesChars;

  const verdict = categorize(text, showCategory);
  const confident = verdict.confidence === 'high' || verdict.confidence === 'medium';

  if (confident) {
    return {
      category: verdict.category,
      categorySource: verdict.category === showCategory ? 'show-confirmed' : 'episode',
      categoryConfidence: verdict.confidence,
      categoryMatched: verdict.matched.slice(0, 4),
    };
  }
  if (thin) {
    return {
      category: 'general',
      categorySource: 'unclassified-thin',
      categoryConfidence: verdict.confidence,
      categoryMatched: [],
    };
  }
