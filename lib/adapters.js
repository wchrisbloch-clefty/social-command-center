...
export function normalizeSignal(raw = {}, ctx = {}) {
  ...
  return {
    ...
    podcast: platform === 'Podcast' ? {
      ...
      // An episode carries its OWN topic category (see classifyEpisode in
      // app/api/podcasts/route.js), so the same signal renders in the Podcasts
      // tab AND in the category feed it belongs to. These say how that category
      // was arrived at, so a surprising placement is inspectable:
      //   'episode'            classified from the show notes
      //   'show-confirmed'     classified, and it agreed with the show
      //   'show'               text was inconclusive, the show's category stands
      //   'unclassified-thin'  too little text to make any call — filed general
      categorySource: raw.categorySource || null,
      categoryConfidence: raw.categoryConfidence || null,
      categoryMatched: Array.isArray(raw.categoryMatched) ? raw.categoryMatched.slice(0, 4) : [],
    } : null,
  };
}
 ...
 * One signal per id.
 *
 * Podcast episodes reach the feed through /api/podcasts AND appear in the
 * Podcasts tab, and a topic source can produce the same URL as a followed
 * account. Both are the same signal seen twice, never two signals — and a
 * duplicate is not merely a repeated card: it double-counts in every velocity
 * total, every category bar and every cross-show topic, so the numbers stop
 * matching the list.
 *
 * FIRST occurrence wins, so callers can order by preference before deduping.
 */
...
export async function getFeed({ category = null, signal } = {}) {
  ...
  // Podcasts join the feed here. An episode is a signal like any other: it has
  // a topic category of its own, it ranks by the same velocity engine, and it
  // keeps tier 'podcast' so it stays identifiable inside a topic tab. The
  // Podcasts tab fetches the same route separately — that is two VIEWS of one
  // signal, not two signals, because both render objects with the same id.
  const [social, youtube, podcasts] = await Promise.all([
    getJson(`/api/social${qs}`, signal),
    getJson(`/api/youtube${qs}`, signal),
    getJson(`/api/podcasts${qs}`, signal),
  ]);
  ...
  //
  // Then dedupe. Nothing today is expected to produce the same id twice, which
  // is exactly why this is here: a duplicate would be silent, and it would
  // double-count in every velocity total and category bar before anyone
  // noticed the card was repeated.
  const items = sortSignals(dedupeById(raw.map(item => normalizeSignal(item))));
