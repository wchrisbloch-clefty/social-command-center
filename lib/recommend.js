...
export function configLineFor(rec, { category, date = new Date() } = {}) {
  ...
  // A podcast is keyed by feed URL, not a handle — there is no handle to have.
  if (rec.platform === 'Podcast' && rec.feedUrl) {
    return {
      array: 'PODCAST_SOURCES',
      line: `  // ${why}\n  { platform: 'Podcast', show: ${JSON.stringify(rec.show || rec.display)}, feedUrl: ${JSON.stringify(rec.feedUrl)}, label: ${JSON.stringify(rec.display)}, category: '${cat}', limit: 4 },`,
    };
  }
